    ...
    def __init__(self, in_channel, out_channel, kernel_size, stride, bias=True, norm=False, relu=True, transpose=False):
        ...
        if transpose:
            ...
            layers.append(
                ...
                CSU(in_channel, out_channel, kernel_size, padding=padding, stride=stride, bias=bias)
                )
...
class cubic_attention(nn.Module):
    def __init__(self, dim, group, kernel) -> None:
        ...
        self.H_spatial_att = nn.Sequential(
                                    spatial_strip_att_dilated(dim, group=group, kernel=kernel, dilated=1),
                                    spatial_strip_att_dilated(dim, group=group, kernel=kernel, dilated=(kernel+1)//2)
                                    ) # dilated=(kernel+1)//2
        self.W_spatial_att = nn.Sequential(
                                    spatial_strip_att_dilated(dim, group=group, kernel=kernel, H=False, dilated=1),
