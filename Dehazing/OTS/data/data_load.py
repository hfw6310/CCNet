...
def train_dataloader(path, batch_size=64, num_workers=0, RandomCropSize=0, C=256):
    image_dir = os.path.join(path, 'train')

    dataloader = DataLoader(
        DeblurDataset(image_dir, ps=RandomCropSize),
...
class DeblurDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.ps is not None:
            ...
            # print('image:',np.array(image))
            # print('label:',np.array(label))
            image = F.to_tensor(image)     # normalize
            label = F.to_tensor(label)
