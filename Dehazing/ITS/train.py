...
def _train(model, args):
    ...
    dataloader = train_dataloader(args.data_dir, args.batch_size, args.num_worker, args.RandomCropSize)
    ...
    # inputs = torch.randn(3, 256, 256).to(device)
    # stat(model, inputs)

    flops, params = profile(model, (inputs, ))
    print(f'Network: , with flops(256 x 256): {flops}, with active parameters: {params/1e3} K.')
    print(f'Network: , with flops(256 x 256): {flops/1e9:.9f} GMac, with active parameters: {params/1e3} K.')
    logfile = open(log_path, "a")
    logfile.write(f'Network:  with flops(256 x 256): {flops/1e9:.9f} GMac, with active parameters: {params/1e3} K \n')
    logfile.close()
    ...
    for epoch_idx in range(epoch, args.num_epoch + 1):
        ...
        for iter_idx, batch_data in enumerate(dataloader):
            ...
            ### for new network
            # label_img2 = F.interpolate(label_img, scale_factor=2, mode='bilinear')
            # label_img4 = F.interpolate(label_img, scale_factor=4, mode='bilinear')
        ...
        if epoch_idx % args.valid_freq == 0:
            ...
            print('%03d epoch \nAverage PSNR %.2f dB' % (epoch_idx, val), ",   ",
                            'Best PSNR %.2f dB\n' % best_psnr)
            ##### add log     
            logfile.write('%03d epoch \n Average PSNR %.2f dB' % (epoch_idx, val) + ",   " + 'Best PSNR %.2f dB\n' % best_psnr) 
        logfile.close()
